    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto">
            ...
            <Card className="border-dashed border-2 border-dark-border shadow-none bg-surface">
                {!file ? (
                    <div
                        className={`flex flex-col items-center justify-center py-16 px-4 transition-colors duration-200 cursor-pointer ${isDragging ? "bg-accent-subtle border-accent" : "hover:bg-[#141414]"}`}
            ...
            {result && result.orders.length > 0 && (
                <Card title="Imported Orders" description={`${result.imported} orders were successfully imported.`}>
            ...
            {result && result.errors.length > 0 && (
                <Card title="Failed Rows" description={`${result.failed} rows could not be imported.`}>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-[#1A1A1A]">
                            <thead className="bg-[#0A0A0A]">
                                <tr>
