...
export function OrdersList() {
    ...
    // Client-side search and sort on current page
            ...
            <Card className="p-0 overflow-hidden">
                <div className="p-4 border-b border-dark-border bg-[#0A0A0A] flex flex-col sm:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        <span className="text-sm text-dark-text-tertiary whitespace-nowrap">Status:</span>
                        <select
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full">
                        ...
                        <tbody className="bg-surface">
                            {loading ? (
                                <tr>
                                    <td colSpan={8} className="px-6 py-12 text-center text-dark-text-tertiary">
                                        Loading orders...
                                    </td>
                                </tr>
                            ) : error ? (
                                ...
                                filteredOrders.map((order) => {
                                    ...
                                    return (
                                        <Fragment key={order.id}>
                                            <tr className={`border-b border-[#1A1A1A] transition-colors ${isExpanded ? "bg-[#111111]" : "hover:bg-surface-elevated"}`}>
                                                <td className="px-4 py-4 w-8">
                                                    <button
                                                        onClick={() => toggleRow(order.id)}
                                                        className="text-[#444444] hover:text-accent transition-colors"
                                                    >
                                                        <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isExpanded ? "rotate-180 text-accent" : ""}`} />
                                                    </button>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-accent font-mono">{order.id}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-[#666666]">{order.date}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-[#666666] font-mono">
                                                    <div className="flex flex-col text-xs">
                                                        <span>Lat: {order.lat}</span>
                                                        <span>Lon: {order.lon}</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-white font-mono">${order.subtotal.toFixed(2)}</td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-white font-mono">${order.total.toFixed(2)}</td>
                                                        ...
                                                        <div className="px-8 py-5 border-l-2 border-accent ml-4">
                                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                                                <div className="space-y-3">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Date</p>
                                                                ...
                                                                <div className="space-y-3">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Latitude</p>
                                                                        <p className="text-sm text-white font-mono">{order.lat}</p>
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Longitude</p>
                                                                        <p className="text-sm text-white font-mono">{order.lon}</p>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div className="space-y-3">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Subtotal</p>
                                                                        <p className="text-sm text-white font-mono">${order.subtotal.toFixed(2)}</p>
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Tax Rate</p>
                                                                        <p className="text-sm text-white font-mono">{(order.taxRate * 100).toFixed(2)}%</p>
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-xs text-[#555555] mb-0.5">Tax Amount</p>
                                                                        <p className="text-sm text-[#888888] font-mono">${order.taxAmount.toFixed(2)}</p>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div className="space-y-3">
                                                                    ...
                                                                    <div className="bg-[#141414] rounded-lg p-4 border border-[#222222]">
                                                                        <p className="text-xs text-[#555555] mb-1">Total Due</p>
                                                                        <p className="text-2xl font-bold text-accent font-mono">${order.total.toFixed(2)}</p>
                                                                        <p className="text-xs text-[#444444] mt-2">Incl. ${order.taxAmount.toFixed(2)} tax</p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p className="text-sm text-[#666666]">
                                ...
                                {" "}to{" "}
                                <span className="font-medium text-white">{Math.min(currentPage * itemsPerPage, totalOrders)}</span>
                                {" "}of{" "}
                                ...
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <button
                                        ...
                                        className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                                            currentPage === page
                                                ? "z-10 bg-accent-subtle border-accent text-accent"
                                                : "bg-surface border-dark-border text-[#666666] hover:bg-surface-elevated"
                                        }`}
                                    >
                                        {page}
                                    </button>
